...
- **P7-5 WAL pinned-snapshot read race (sync-insert path)**: `TestTableScopeAllowedTableFiltered` intermittently (~5% under load; first observed in CI 2026-09-12) reads back a row missing the payload columns the sync `db.insert` ALTERs in — the GET's `SELECT *` executes against a pooled connection whose WAL read snapshot predates the schema change; an immediate second GET on another pooled connection always returns the complete row (confirmed: batched-write repro 0/60, sync repro ~5/60). Test currently contains a one-retry guard (`table_scope_test.go`) rather than a root-cause fix. Debug round needs: driver-level instrumentation (go-sqlite3) or a connection-init `PRAGMA data_version` check to find which startup/insert-path query leaves a read transaction open across the ALTER, then fix in the engine (likely in `execWithRetry`/`ensureTable` on the sync path) and remove the retry.
