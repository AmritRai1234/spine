...
- **P7-5 WAL pinned-snapshot read race (sync-insert path)** — ROOT-CAUSE NARROWED 2026-09-12, driver-level; engine-side mitigation available:
  - Symptom: `TestTableScopeAllowedTableFiltered` / `TestTableScopeUnscopedRoleFullRead` intermittently (~8-17% under load) — the GET's `SELECT *` returns the pre-ALTER column set (`_spine_id, created_at`, no payload columns) and only the first matching row, while a second GET and all direct pool reads return the complete result. Both scope-leak assertions still pass.
  - Proven by instrumentation (gated `SPINE_DEBUG_QROWS` log in `queryRows`): `rows.Columns()` itself returns the stale 2-column list for the identical SQL that returns 3 columns on other connections — i.e., the driver hands back a prepared statement compiled against the pre-ALTER schema. Driver stmt cache is OFF (no `_stmt_cache_size` in DSN; go-sqlite3 `stmtCacheEnabled=false`); a brand-new pooled connection still goes stale (SetMaxIdleConns(0) experiment: 6/120); the DB FILE is always correct (verified via a separate read-only handle at failure time: 3 cols, 2 rows, correct emails). Direct `b.db.Query` from the test goroutine never fails.
  - Conclusion: a go-sqlite3/WAL-level schema-cookie staleness — a connection whose first statement runs while the runtime `ALTER TABLE ADD COLUMN` commit is in flight caches the old schema and (apparently) keeps serving it; `sqlite3_prepare_v2`'s auto-reprepare does not save it. This is driver interaction territory, not an engine SQL bug — the engine's own file state is always correct.
  - ENGINE-SIDE MITIGATION (preferred fix): eliminate runtime ALTERs entirely — at startup, `EnsureTables` should derive each table's FULL column set from the manifest (all node payload field types for events that write to the table), so `ensureTable`'s hot-path CREATE-then-ALTER sequence never happens and the schema is stable before the first request. Fallback mitigation: retry-once guard in tests (current state).
  - Debug-round checklist: reproduce with `-race` + `SPINE_DEBUG_QROWS=1`; test whether `PRAGMA schema_version` polling on a fresh conn shows the stale value; try go-sqlite3 `-DSQLITE_MAX_...`/other builds or modernc.org/sqlite as a comparison driver; consider upstreaming a repro to mattn/go-sqlite3 (v1.14.48 has a custom `_sqlite3_prepare_v2_blocking` hook that may interact with WAL reader refresh).
  - UP-FRONT DECISIONS for the fix round (EnsureTables contract change — touches every app's startup): (1) Column typing source: manifest-declared field types (authoritative, from node payload contracts) vs. inferred from first-seen values — prefer declared, since inference reintroduces the type-zoo problem. (2) Dynamic/free-form payloads with no fixed contract: decide whether undeclared-but-inserted columns still get the lazy-ALTER path (race window remains for them) or whether manifests must declare every column up front (breaking; strictest). (3) Migration story for existing deployed DBs built the lazy-ALTER way: startup schema reconciliation (ALTER-to-contract at startup only, before traffic) vs. version-gated rebuild. (4) Multi-event tables: columns must union across ALL events writing to a table — needs a registry helper to aggregate field types per table. (5) Extension columns created via schema evolution at runtime (declared in manifest comments as "optional extension columns ride along") need either up-front declaration in nodes.emits or an explicit opt-in for free-form columns.
